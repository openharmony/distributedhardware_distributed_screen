...
int32_t ScreenClient::AddWindow(std::shared_ptr<WindowProperty> &windowProperty)
{
    ...
    DHLOGI("Add window ID = %{public}" PRId32 " success.", windowId);
    return windowId;
}
...
{
    {
        ...
        if (iter == surfaceMap_.end()) {
            DHLOGE("windowId ID = %{public}" PRId32 " is non-existent.", windowId);
            return ERR_DH_SCREEN_SCREENCLIENT_SHOW_WINDOW_ERROR;
        }
    }
    int32_t ret = ScreenClientWindowAdapter::GetInstance().ShowWindow(windowId);
    if (ret != DH_SUCCESS) {
        DHLOGE("Show window ID = %{public}" PRId32 " failed.", windowId);
        return ret;
    }
    DHLOGI("Show window ID = %{public}" PRId32 " success.", windowId);
    return ret;
}
...
int32_t ScreenClient::HideWindow(int32_t windowId)
{
    ...
    if (ret != DH_SUCCESS) {
        DHLOGE("Hide window ID = %{public}" PRId32 " failed.", windowId);
        return ret;
    }
    DHLOGI("Hide window ID = %{public}" PRId32 " success.", windowId);
    return ret;
}
...
int32_t ScreenClient::MoveWindow(int32_t windowId, int32_t startX, int32_t startY)
{
    ...
    if (ret != DH_SUCCESS) {
        DHLOGE("Move window ID = %{public}" PRId32 " failed.", windowId);
        return ret;
    }
    DHLOGD("Move window ID = %{public}" PRId32 " success.", windowId);
    return ret;
}
...
sptr<Surface> ScreenClient::GetSurface(int32_t windowId)
{
    ...
    DHLOGD("Get surface ID = %{public}" PRId32 " success.", windowId);
    return surface;
}
...
int32_t ScreenClient::RemoveWindow(int32_t windowId)
{
    ...
    if (ret != DH_SUCCESS) {
        DHLOGE("windowId ID = %{public}" PRId32 " remove failed.", windowId);
        return ret;
    }
    DHLOGD("windowId ID = %{public}" PRId32 " remove success.", windowId);
    return ret;
}
